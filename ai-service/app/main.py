...
async def chat_endpoint(request: ChatRequest):
    try:
        ...
        if "recommendations" in result and result["recommendations"]:
            return ChatResponse(response=result["recommendations"], is_json=True)

        # Fallback to general chat response parsing
        if "messages" in result and len(result["messages"]) > 0:
            last_message = result["messages"][-1]
            return ChatResponse(response=last_message.content, is_json=False)
        ...
        raise HTTPException(
            status_code=429, detail=f"Groq API rate limit exceeded: {str(e)}"
        )
